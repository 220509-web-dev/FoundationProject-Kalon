...
--establish a 'namespace' for the related database entities to exist within
create schema shoe_collection_app;

--reference the schema created above
set search_path to shoe_collection_app;

create table user_account_level (
...
references user_account_level(id);
...
create table shoes (
    ...
    owner_id int,
    category_id int default 1 check (category_id > 0 and category_id <= 6),
